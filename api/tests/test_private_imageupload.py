...
class APIPrivateImageUploadTestCase(APITestCase, TenantTestCase):
    fixtures = [
        # 'groups',
        # 'permissions',
    ]
    ...
    def setUp(self):
        ...
        super(APIPrivateImageUploadTestCase, self).setUp()
        ...
        # Initialize our test data.
        PrivateImageUpload.objects.bulk_create([
            PrivateImageUpload(owner=self.user),
            PrivateImageUpload(owner=self.user),
            PrivateImageUpload(owner=self.user),
        ])
    ...
    def tearDown(self):
        ...
        image_uploads = PrivateImageUpload.objects.all()
        for an_image in image_uploads:
            an_image.delete()
    ...
    def test_to_string(self):
        ...
        privateimageupload = PrivateImageUpload.objects.create(
            id=2030,
            owner=self.user,
        )
        privateimageupload.save()
        self.assertEqual(str(privateimageupload), "")

    @transaction.atomic
    def test_post_with_authenticated_user(self):
        # Step 1: Set the location variables.
        url = reverse('privateimageupload-list')
    ...
    def test_put_with_authenticated_user(self):
        ...
        uploads = PrivateImageUpload.objects.all()
        for upload in uploads.all():
            upload.delete()

        # Create a new object with our specific test data.
        PrivateImageUpload.objects.create(id=1, owner_id=self.user.id,)

        # Run the test.
        url = reverse('privateimageupload-list')+'1/'
    ...
    def test_put_with_different_owner(self):
        ...
        # Step 1: Log in and get the first PrivateImageUpload object.
    ...
    def test_delete_with_different_owner(self):
        # Step 1: Initialize our test user.
        user = User.objects.create_user(  # Create our user.
            email='h@h.com',
            username='h@h.com',
            password=TEST_USER_PASSWORD
