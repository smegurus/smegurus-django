...
class APIBusinessIdeaWithTenantSchemaTestCase(APITestCase, TenantTestCase):
    fixtures = []
    ...
    def setUp(self):
        ...
        super(APIBusinessIdeaWithTenantSchemaTestCase, self).setUp()
    ...
    @transaction.atomic
    def test_list(self):
        response = self.unauthorized_client.get('/api/tenantbusinessidea/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    @transaction.atomic
    def test_list_with_authentication(self):
        response = self.authorized_client.get('/api/tenantbusinessidea/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_post(self):
        ...
        response = self.unauthorized_client.post('/api/tenantbusinessidea/', json.dumps(data), content_type='application/json')
    ...
    def test_post_with_authentication(self):
        ...
        response = self.authorized_client.post('/api/tenantbusinessidea/', json.dumps(data), content_type='application/json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    @transaction.atomic
    def test_put(self):
        # Delete any previous data.
        postal_addesses = BusinessIdea.objects.all()
        ...
        response = self.unauthorized_client.put('/api/tenantbusinessidea/1/', json.dumps(data), content_type='application/json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_delete(self):
        response = self.unauthorized_client.delete('/api/tenantbusinessidea/1/')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
