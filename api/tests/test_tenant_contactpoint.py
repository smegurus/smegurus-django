...
class APIContactPointTestCase(APITestCase, TenantTestCase):
    ...
    @transaction.atomic
    def test_list(self):
        response = self.unauthorized_client.get('/api/contactpoint/')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    @transaction.atomic
    def test_list_with_authentication(self):
        response = self.authorized_client.get('/api/contactpoint/')
    ...
    def test_post(self):
        ...
        response = self.unauthorized_client.post('/api/contactpoint/')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_post_with_authentication(self):
        ...
        response = self.authorized_client.post('/api/contactpoint/', data, format="json")
    ...
    def test_put(self):
        ...
        response = self.unauthorized_client.put('/api/contactpoint/1/', json.dumps(data), content_type='application/json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_put_with_authorization(self):
        ...
        response = self.authorized_client.put('/api/contactpoint/1/', json.dumps(data), content_type='application/json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    @transaction.atomic
    def test_delete(self):
        response = self.unauthorized_client.delete('/api/contactpoint/1/')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    @transaction.atomic
    def test_delete_with_authentication(self):
        response = self.authorized_client.delete('/api/contactpoint/1/')
