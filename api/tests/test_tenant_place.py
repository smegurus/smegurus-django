...
class APIPlaceTestCase(APITestCase, TenantTestCase):
    ...
    @transaction.atomic
    def test_list(self):
        response = self.unauthorized_client.get('/api/place/')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    @transaction.atomic
    def test_list_with_authentication(self):
        response = self.authorized_client.get('/api/place/')
    ...
    def test_post(self):
        ...
        response = self.unauthorized_client.post('/api/place/')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_post_with_authentication(self):
        ...
        response = self.authorized_client.post('/api/place/', json.dumps(data), content_type='application/json')
    ...
    def test_put(self):
        ...
        response = self.unauthorized_client.put('/api/place/1/', json.dumps(data), content_type='application/json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_put_with_authorization(self):
        ...
        response = self.authorized_client.put('/api/place/1/', json.dumps(data), content_type='application/json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    @transaction.atomic
    def test_delete(self):
        response = self.unauthorized_client.delete('/api/place/1/')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    @transaction.atomic
    def test_delete_with_authentication(self):
        response = self.authorized_client.delete('/api/place/1/')
