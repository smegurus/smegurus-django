...
class TenantLearningTestCases(APITestCase, TenantTestCase):
    fixtures = []
    ...
    def setUp(self):
        ...
        super(TenantLearningTestCases, self).setUp()
    ...
    def tearDown(self):
        ...
        for user in users.all():
            user.delete()
        # super(TenantLearningTestCases, self).tearDown()

    @transaction.atomic
    def test_learning_page(self):
        url = reverse('tenant_learning')
        response = self.authorized_client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(len(response.content) > 1)
        self.assertIn(b'Learning',response.content)
