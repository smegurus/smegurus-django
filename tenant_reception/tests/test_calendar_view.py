...
class TenantReceptionCalendarTestCases(APITestCase, TenantTestCase):
    fixtures = []
    ...
    def setUp(self):
        ...
        super(TenantReceptionCalendarTestCases, self).setUp()
    ...
    def tearDown(self):
        ...
        for user in users.all():
            user.delete()
        # super(TenantReceptionCalendarTestCases, self).tearDown()

    @transaction.atomic
    def test_calendar_master_page(self):
        url = reverse('tenant_reception_calendar_master')
        response = self.authorized_client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(len(response.content) > 1)
        self.assertIn(b'My Calendar',response.content)
        self.assertIn(b'My Events',response.content)
    ...
    def test_calendar_details_page(self):
        ...
        url = reverse('tenant_reception_calendar_details', args=[calendar_event.id,])
        response = self.authorized_client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(len(response.content) > 1)
